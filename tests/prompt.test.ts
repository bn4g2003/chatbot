...
test("roleplay prompt preserves priority, boundaries, and character agency", () => {
  ...
  assert.match(prompt, /Canon facts below outrank memory/); assert.match(prompt, /Never write the user's dialogue/); assert.match(prompt, /Vietnamese/); assert.match(prompt, /return a book/); assert.match(prompt, /character may disagree/); assert.match(prompt, /Do not describe the setting in every reply/);
});
